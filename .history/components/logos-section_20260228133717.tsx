  ...
  const logos = [
    {
      ...
      bgColor: 'bg-white', // Switched to white so logos actually pop
    },
    {
      name: 'AWS Cloud Club',
        ...
        <div className="flex items-start justify-start gap-12 sm:gap-16 lg:gap-20">
          {logos.map((logo, index) => (
            <div key={index} className="flex flex-col items-center gap-3 flex-1">
              <div
                className={`h-16 w-16 sm:h-20 sm:w-20 rounded-lg ${logo.bgColor} flex items-center justify-center shadow-sm flex-shrink-0 overflow-hidden p-2 border border-gray-100`}
              >
                <img 
                  src={logo.src} 
                  alt={logo.name} 
                  ...
                  onError={(e) => {
                    e.currentTarget.src = "https://placehold.co/80?text=Logo+Missing"
                  }}
                />
              </div>
